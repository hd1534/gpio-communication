...
// 라즈베리 파이의 GPIO 컨트롤러(BCM2835)의 기본 번호. 커널 내부에서 GPIO를
// 식별할 때 사용.
...
struct gpio_entry {
  ...
  struct gpio_desc
      *desc;  // 해당 GPIO를 가리키는 디스크립터 (최신 gpiod 인터페이스의 핵심)

  // struct device *dev; // 이 GPIO에 할당된 디바이스 구조체 (/dev 및 /sys
  // 파일과 연결됨)
...
static void gpio_set_4bit(char data_bits) {
  ...
  gpiod_set_value(master_dev->datapins[3],
                  (data_bits >> 3) &
                      0x01);  // master는 slave의 clock을 data 선으로 사용한다.
}
...
static ssize_t gpio_comm_write(struct file *filp, const char __user *buf,
  ...
  for (i = 0; i < dev->tx_len; i++) {
    ...
    set_data_pins(byte_to_send & 0x0F);

    toggle_clock();

    set_data_pins((byte_to_send >> 4) & 0x0F);
...
static ssize_t gpio_comm_read(struct file *filp, char __user *buf, size_t len,
                              loff_t *off) {
  struct gpio_comm_dev *dev = filp->private_data;
  ...
  mod_timer(&dev->timeout_timer,
            jiffies + msecs_to_jiffies(TIMEOUT_MS * 5));  // 첫 대기는 길게
  ...
  ret = wait_event_interruptible(
      dev->wq, dev->state == COMM_STATE_DONE || dev->state == COMM_STATE_ERROR);
  ...
  received_crc =
      (dev->rx_buffer[bytes_to_copy + 1] << 8) | dev->rx_buffer[bytes_to_copy];
...
static ssize_t assign_gpio(int bcm, struct gpio_entry **entry_out) {
  ...
  if (used_gpio_bcm[bcm]) return -EEXIST;
...
static ssize_t connect_store(const struct class *class,
  ...
  // 0. 값 읽기
  if (sscanf(buf, "%s,%s,%d,%d,%d,%d,%d", name, mode, bcm, bcm + 1, bcm + 2,
             bcm + 3, bcm + 4) != 6) {
    return -EINVAL;
  }
  ...
  ret = request_irq(new_dev->irq, comm_irq_handler, IRQF_TRIGGER_RISING,
                    DRIVER_NAME, new_dev);
  ...
  if (connect_mode == READ_WRITE) {
    // TODO: 모든 핀을 확인해서 idle 상태인거 확인하고, d0~d3중에 비어있는거
    // 하나에 접근
  }
  ...
  pr_info(
      "[%s] %s device initialized with pins M_CTRL: %d; D0-4: %d,%d,%d,%d;\n",
...
static ssize_t disconnect_store(const struct class *class,
                                const struct class_attribute *attr,
                                const char *buf, size_t count) {
  int idx;
...
static int __init gpio_comm_init(void) {
  ...
  pr_info("[%s] module loading (mode: %s)\n", DRIVER_NAME,
          (mode == COMM_MODE_MASTER) ? "Master" : "Slave");
  ...
  g_dev->device =
      device_create(g_dev_class, NULL, g_master_dev_num, NULL, DEVICE_NAME);
